def parse_mime_table()
  ...
  text = "Extension 	Kind of document 	MIME Type
  ...
  text.each_line do |line|
    line = line.strip
    next if line.empty?
    next if line.start_with?("Extension") # skip header

    # Split on tabs (the table uses tabs)
    columns = line.split("\t")
    ...
    # Clean MIME types (remove notes after periods)
    mime_types = mime_raw
      .split(",")
      .map { |m| m.split(".").first.strip }

    # Handle multiple extensions like ".jpg, .jpeg"
    extensions_raw.split(",").each do |ext|
      clean_ext = ext.strip.sub(".", "")
      mime_hash[clean_ext] =
        mime_types.length == 1 ? mime_types.first : mime_types
    end
  end
